    ...
    for epoch in range(num_epoch):
        ...
        for data in train_dataloader:
            ...
            with torch.no_grad():
                ...
                edges = edges.permute(2, 0, 1).reshape(2, -1).cuda()
        ...
        if epoch % save_every_n_epoch == 0 or epoch == num_epoch - 1:
            mean_loss = loss_accumulator / counter
            print("Epoch", epoch, "mean loss while evaluation", format(mean_loss, ".3e"))

        if (epoch % save_every_n_epoch == 0 or epoch == num_epoch - 1) and epoch != 0:
            name = "model_" + str(epoch) + ".pth"
