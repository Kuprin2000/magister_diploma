...
static int trajectory = 0;
static int step = 0;
static const int save_every_steps = 1;
...
namespace SystemParam {
	static const int n = 17; // must be odd, n * n = n_vertices | 61
	static const float w = 2.0f; // width | 2.0f
	static const float h = 0.002f; // time step, smaller for better results | 0.008f = 0.016f/2
...
static void demo_hang() {
	...
	std::mt19937 gen(rd());
	std::uniform_int_distribution<> dist_1(1, 5);
	int num_fixed_points = dist_1(gen);

	std::uniform_int_distribution<> dist_2(1, n * n);
	for (int i = 0; i < num_fixed_points; ++i)
	{
		cornerFixer->fixPoint(dist_2(gen));
	}
...
static void animateCloth() {
	...
	if (trajectory == num_trajectories)
	{
		exit(0);
	}
	...
	if (step == num_steps + 1)
	{
		...
		++trajectory;
		step = 0;
		std::cout << "Trajectory " << trajectory << " done, " << std::fixed << std::setprecision(2) << ((float)trajectory / (float)num_trajectories * 100) << "%" << std::endl;
		return;
	}
	...
	if (step == 0)
	{
		const int num_vertices = g_clothMesh->n_vertices();
		std::filesystem::path path_1(std::format("./trajectories/{}/indices.npy", trajectory));
		std::filesystem::path parent_dir_1 = path_1.parent_path();
		if (!parent_dir_1.empty() && !std::filesystem::exists(parent_dir_1)) {
			std::filesystem::create_directories(parent_dir_1);
		}
		cnpy::npy_save(path_1.string(), g_clothMesh->ibuff(), { (unsigned long long)g_clothMesh->ibuffLen(),3u,1u }, "w");

		const mass_spring_system::EdgeList& cloth_edges = g_solver->getEdges();
		std::vector<unsigned int> edges;
		edges.reserve(2 * cloth_edges.size());

		for (const auto& elem : cloth_edges)
		{
			edges.push_back(elem.first);
			edges.push_back(elem.second);
		}

		std::filesystem::path path_2(std::format("./trajectories/{}/edges.npy", trajectory));
		std::filesystem::path parent_dir_2 = path_2.parent_path();
		if (!parent_dir_2.empty() && !std::filesystem::exists(parent_dir_2)) {
			std::filesystem::create_directories(parent_dir_2);
		}
		cnpy::npy_save(path_2.string(), edges.data(), { (unsigned long long)cloth_edges.size(),2u,1u }, "w");
	}

	if (step % save_every_steps == 0 || step == num_steps)
	{
		const int num_vertices = g_clothMesh->n_vertices();
		const MassSpringSolver::VectorXf& prev_state = g_solver->getPrevState();
		const MassSpringSolver::Map& cur_state = g_solver->getCurrentState();
		const float time_step = g_solver->getTimeStep();

		std::vector<float> positions;
		positions.reserve(3 * num_vertices);

		std::vector<float> directions;
		directions.reserve(3 * num_vertices);

		for (int i = 0; i < 3 * num_vertices; ++i)
		{
			const float prev_pos = prev_state[i];
			const float cur_pos = cur_state[i];

			positions.push_back(cur_pos);
			directions.push_back((cur_pos - prev_pos) / time_step);
		}

		std::filesystem::path path_1(std::format("./trajectories/{}/position_{}.npy", trajectory, step));
		std::filesystem::path parent_dir_1 = path_1.parent_path();
		if (!parent_dir_1.empty() && !std::filesystem::exists(parent_dir_1)) {
			std::filesystem::create_directories(parent_dir_1);
		}
		cnpy::npy_save(path_1.string(), positions.data(), { (unsigned long long)num_vertices,3u,1u }, "w");

		std::filesystem::path path_2(std::format("./trajectories/{}/direction_{}.npy", trajectory, step));
		std::filesystem::path parent_dir_2 = path_2.parent_path();
		if (!parent_dir_2.empty() && !std::filesystem::exists(parent_dir_2)) {
			std::filesystem::create_directories(parent_dir_2);
		}
		cnpy::npy_save(path_2.string(), directions.data(), { (unsigned long long)num_vertices,3u,1u }, "w");
	}
